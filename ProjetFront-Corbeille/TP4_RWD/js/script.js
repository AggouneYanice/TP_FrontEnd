...
function afficherJoueurs() {
    ...
    for (i in joueurs) {
        var nouvelleDiv = $('<div id="contain_'+i+'"></div>');
        $('#container').append(nouvelleDiv);
        var pImg = $('<p><img height="70" width="50" alt="'+joueurs[i].nom+'" src="'+joueurs[i].image+'" '+joueurs[i].nom+'/></p>');
        var pNom = $('<p>'+joueurs[i].nom+'</p>');
        var pPoste = $('<p>'+joueurs[i].poste+'</p>');
        var pDesc = $('<p>'+joueurs[i].description+'</p>');
        var pButtonAgrandir = $('<p><button class="bouton_transition" type="button" name="agrandir">Agrandir</button></p>');
        var idJoueur = i.substring(i.length -1, i.length);
        var pButtonModifier = $('<p><button class="bouton_transition" type="button" name="modifier" id="bt_modifier_'+idJoueur+'" data-id="'+i+'">Modifier</button></p>');
        var pButtonSupprimer = $('<p><button class="bouton_transition" type="button" name="supprimer" id="bt_supprimer_'+idJoueur+'" data-id="'+idJoueur+'">Supprimer</button></p>');
        var divBoutons = $('<div id="containBt_'+i+'"></div>');
        $('#contain_'+i).append(divBoutons);
        $('#containBt_'+i).append(pButtonAgrandir)
            .append(pButtonModifier)
            .append(pButtonSupprimer);
        $('#contain_'+i).append(pImg)
            .append(pNom)
            .append(pPoste)
            .append(pDesc);
    }
    ...
    for (i in menu) {
        ...
        if (menu[i].fils.length !== 0) {
            ...
            for (j in menu[i].fils) {
                ...
                var nouveauSousLien = $('<a href="index.html">'+menu[i].fils[j]+'</a>');
...
function ajouterJoueur() {
    ...
    console.log(obe);
    if (nbJoueurs===1) {
        joueursString = JSON.stringify(joueurs);
...
function deployMenu() {
    if ($('div[id^=menu]').hasClass('visible')) {
        $('div[id^=menu]').addClass('unvisible')
            .removeClass('visible')
            .css('visibility', 'hidden');
    } else {
        $('div[id^=menu]').addClass('visible')
